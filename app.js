(() => {
  ...
  // RuneScape-like XP curve (approximate RS1/OSRS formula).
  // xpForLevel(1) = 0, and grows quickly toward 99.
  ...
  function progressToNext(xp) {
    ...
    if (lvl >= MAX_LEVEL) {
      return { level: MAX_LEVEL, pct: 100, cur: Math.floor(xp), nextXp: Math.floor(xp), toNext: 0 };
    }
    const curMin = xpForLevel(lvl);
    const nextMin = xpForLevel(lvl + 1);
    const curXp = Math.max(0, Math.floor(xp));
    const pct = ((curXp - curMin) / (nextMin - curMin)) * 100;
    return {
      level: lvl,
      pct: clamp(pct, 0, 100),
      cur: curXp,
      nextXp: nextMin,
      toNext: Math.max(0, nextMin - curXp),
    };
  }
  ...
  function renderDetail() {
    ...
    if (recents.length === 0) {
      recentList.replaceChildren(
        el("div", { class: "detailEmpty" }, [
          el("div", { text: "No recent activity yet." }),
        ])
      );
    } else {
      recentList.replaceChildren(
  ...
  function openLogDialog(defaultXp = null) {
    ...
    if (defaultXp != null) xpInput.value = String(defaultXp);
    else xpInput.value = "";
    noteInput.value = "";
  ...
  function hydrateSkillSelect() {
    skillSelect.replaceChildren(
      ...SKILLS.map((s) => el("option", { value: s, text: s }))
    );
  }
  ...
  function bindEvents() {
    ...
    // If user changes skill in dialog, keep selection in sync.
    skillSelect.addEventListener("change", () => {
      selectedSkill = skillSelect.value;
      render();
    });

    // Quick add chips on detail card.
    ...
    // Close dialog on outside click (small UX improvement).
